SELECT
	...
	CASE
		When INF.total_bytes_processed is Null Then 0
		ELSE
  Round(INF.total_bytes_billed /(1000 * 1000 * 1000), 3)
	End as Data_Billed_GB,
	CASE
		When contains_substr(INF.query, "as_agg") then 1
		ELSE 0
	End as agg_hit,
  destination_table.project_id,
	destination_table.table_id,
	CASE
		When contains_substr(destination_table.table_id, "as_agg_") then 1
		else 0
	end as atscale_build
FROM
	`region-EU`.INFORMATION_SCHEMA.JOBS INF
WHERE
	INF.user_email in('atscale@semantic-project.iam.gserviceaccount.com')
	and 
